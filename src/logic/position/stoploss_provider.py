...
class StoplossProvider:
    ...
    @staticmethod
    def small_blocks_refined_05_percent_no_trailing(
        position: Position,
    ) -> NDArray[float64]:
        """
        Blocks between 0 and 2% don't use the base block height for the stoploss, and instead
        use 0.5% of the price as the base height.
        """
        height_percentage = position.base_block.height_percentage
        stoploss_coeff = config.stoploss_coeff
    ...
    @staticmethod
    def small_blocks_refined_05_percent_trailing_breakeven_t1(
        position: Position,
    ) -> NDArray[float64]:
        """
        Blocks between 0 and 2% don't use the base block height for the stoploss, and instead
        use 0.5% of the price as the base height. This function also utilizes trailing-breakeven
        stoploss setup, moving the stoploss to the entry after the first target is hit.
        """
        height_percentage = position.base_block.height_percentage
        stoploss_coeff = config.stoploss_coeff
