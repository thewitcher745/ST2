...
class PositionSimulator:
    ...
    def _calculate_temporal_events(pos: Position, klines_data: KLinesData):
        ...
        lows, highs, index_offset = PositionSimulator._get_window_slices(pos, klines_data)
        
        # 2. Find Entry and Stoploss indices
        entry_index, stop_index = PositionSimulator._find_primary_events(pos, lows, highs, index_offset)
        ...
        # 3. Process Targets
        PositionSimulator._process_targets(pos, klines_data, entry_index, stop_index, index_offset)
        
        # 4. Finalize Status
        PositionSimulator._finalize_status(pos)

    @staticmethod
    def _get_window_slices(pos: Position, klines_data: KLinesData) -> tuple[ndarray, ndarray, int]:
        """Extracts the relevant price slices based on block indices."""
    ...
    @staticmethod
    def _find_primary_events(pos: Position, lows: ndarray, highs: ndarray, offset: int) -> tuple[int | None, int | None]:
        """Locates the first candle to hit entry and stoploss."""
        if pos.type == "long":
            entry_cond = lows <= pos.entry
            stop_cond = lows <= pos.stoploss
        else:
            entry_cond = highs >= pos.entry
            stop_cond = highs >= pos.stoploss
    ...
    @staticmethod
    def _process_targets(pos: Position, klines: KLinesData, entry_index: int, stop_index: int | None, offset: int):
        ...
        for target in pos.targets:
            hits = where(current_window >= target)[0] if pos.type == "long" else where(current_window <= target)[0]
            ...
            if hit_local_index == 0:
                if not PositionSimulator._is_valid_same_candle_hit(pos, klines, current_global_offset):
                    if len(hits) > 1:
            ...
            pos.target_times.append(klines.time[hit_local_index + current_global_offset])
    ...
    @staticmethod
    def _is_valid_same_candle_hit(pos: Position, klines: KLinesData, index: int) -> bool:
    ...
    def _calculate_financial_outcome(pos: Position):
        ...
        delta = sum(pos.targets[:hits] * qty_per_target) + (misses * qty_per_target * pos.stoploss)
        if pos.type == "long":
